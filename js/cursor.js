...
const initHovers = () => {
  ...
  const handleMouseClickDraggable = e => {
    jQuery(innerCursor).addClass("expand-cursor");
    setTimeout(function() {
      jQuery(innerCursor).removeClass("expand-cursor");
    }, 500);
  };
  ...
  const handleMouseEnterNormal = e => {
    jQuery(innerCursor).removeClass("enterDraggable");
    jQuery(canvas).removeClass("hidden");
  };
  const handleMouseLeaveNormal = e => {
    jQuery(innerCursor).addClass("enterDraggable");
    jQuery(canvas).addClass("hidden");
  };
  const handleMouseEnterNormalLink = e => {
    jQuery(innerCursor).addClass("enterLinks");
    jQuery(innerCursor).removeClass("enterDraggable");
    jQuery(canvas).addClass("hidden");
  };
  const handleMouseLeaveNormalLink = e => {
    jQuery(innerCursor).removeClass("enterLinks");
    jQuery(innerCursor).addClass("enterDraggable");
    jQuery(canvas).removeClass("hidden");	};
  ...
  // 	}
  // });
  const links = document.querySelectorAll("a, .cursor-links, .btn, .swiper-pagination, .project-item__link");
  links.forEach(item => {
  ...
  draggables.forEach(item => {
    if(item.getAttribute('listener') !== 'true') {
      ...
      item.addEventListener("click", handleMouseClickDraggable);
      item.setAttribute('listener', 'true');
    }
  });
  const normals = document.querySelectorAll(".normal-text");
  normals.forEach(item => {
    if(item.getAttribute('listener') !== 'true') {
      item.addEventListener("mouseenter", handleMouseEnterNormal);
      item.addEventListener("mouseleave", handleMouseLeaveNormal);
      item.setAttribute('listener', 'true');
    }
  });
  const normalLinks = document.querySelectorAll(".normal-link");
  normalLinks.forEach(item => {
    if(item.getAttribute('listener') !== 'true') {
      item.addEventListener("mouseenter", handleMouseEnterNormalLink);
      item.addEventListener("mouseleave", handleMouseLeaveNormalLink);
      item.setAttribute('listener', 'true');
    }
  });
};
jQuery(document).ready(function($){
  ...
  // setTimeout(function() {
  // 	$('.expand-cursor').removeClass('expand-cursor');
  // }, 500);

  // const draggableButton = document.querySelector('.enterDraggable');
  // document.addEventListener('click', () => {
  // 	draggableButton.classList.add("expand-cursor");
  //
  // 	setTimeout(() => {
